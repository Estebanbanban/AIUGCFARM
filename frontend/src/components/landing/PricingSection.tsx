...
export function PricingSection() {
  return (
    <section id="pricing" className="bg-black py-24 px-4 sm:px-6">
      <div className="max-w-4xl mx-auto">
        <FadeInUp className="text-center mb-14">
          <p className="text-xs uppercase tracking-[0.15em] text-[#555] mb-3">Pricing</p>
            ...
            <ScaleIn key={plan.name} delay={i * 0.1}>
              <div
                className={`relative rounded-2xl p-8 flex flex-col h-full transition-all duration-300 ${
                  plan.highlighted
                    ? "bg-[#111] border border-primary/40 shadow-[0_0_40px_rgba(249,115,22,0.07)]"
              ...
              >
                ...
                <div className="mb-6">
                  ...
                  <p className="text-[#666] text-sm mt-1">{plan.description}</p>
                </div>
                ...
                <ul className="space-y-3 mb-8 flex-1">
                  {plan.features.map((f) => (
                    <li key={f} className="flex items-center gap-2.5">
                      ...
                      <span className="text-sm text-[#999]">{f}</span>
                    </li>
                  ))}
                </ul>
